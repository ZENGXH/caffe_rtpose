...
int main(){
    ...
    while(cursor->valid()) {
        ...
        if(false){
            ...
            char name[100];
            sprintf(name, "COCO_train_%012d_%d.jpg", meta.annolist_index, meta.numOtherPeople+1);
            meta.file_name = name;

            // LOG(INFO) << " testing image save mask_miss & mask_all" ;
            static int count = 0; count ++;
            sprintf(name, "./train2014mask/mask_COCO_train_%012d_%d.jpg", meta.annolist_index, meta.numOtherPeople+1);
            imwrite(name, mask_miss);
            // sprintf(name, "%d_all.jpg", meta.annolist_index);
            // imwrite(name, mask_all);
            char filename[100];
            sprintf(name, "./train2014/COCO_train_%012d_%d.jpg", meta.annolist_index, meta.numOtherPeople+1);
            imwrite(name, img);
        }
        // end img
        count ++;
